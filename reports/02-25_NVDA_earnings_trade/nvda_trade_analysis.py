...
from reports.report_utils import plot_ma200_distance
from reports.report_utils import plot_portfolio_rsi
...
def calculate_historical_sympathy_beta(nvda_df, tickers):
  ...
  nvda_recent = nvda_recent[nvda_recent['Earnings_Date'] != pd.to_datetime(
      '2026-02-25').date()]
  results = []
...
def generate_iv_crush_plot(iv_df):
  ...
  plt.title("NVDA Implied Volatility (IV) Crush Decay",
            fontsize=14,
            fontweight='bold')
  plt.ylabel("Premium Decay from Peak to Close (%)", fontsize=12)
  plt.xticks(rotation=45)
  plt.tight_layout()
  out_path = os.path.join(os.path.dirname(__file__), "plots",
                          "nvda_iv_crush.png")
...
def generate_beta_plot(beta_df):
  ...
  sns.set_theme(style="whitegrid")
  df = beta_df.copy()
  df['Bull'] = df['Historic Bull Case (NVDA +)'].str.replace('%',
                                                             '').astype(float)
  df['Bear'] = df['Historic Bear Case (NVDA -)'].str.replace('%',
                                                             '').astype(float)
  df_melt = df.melt(id_vars='Ticker',
                    value_vars=['Bull', 'Bear'],
                    var_name='Scenario',
                    value_name='Expected Move (%)')
  sns.barplot(x='Ticker',
              y='Expected Move (%)',
              hue='Scenario',
              data=df_melt,
              palette=['#2ecc71', '#e74c3c'])
  plt.title("Portfolio Sympathy Beta Impact (1-Day)",
            fontsize=14,
            fontweight='bold')
  plt.axhline(0, color='black', linewidth=1)
  plt.tight_layout()
  out_path = os.path.join(os.path.dirname(__file__), "plots",
                          "portfolio_sympathy_beta.png")
  plt.savefig(out_path, dpi=300)
  plt.close()
...
def generate_trajectory_prediction_plot():
  ...
  x = [0, 1, 2, 3]
  labels = [
      'T0 (Feb 25 Close)', 'T1 Open (9:30 AM)', 'T1 Peak (10:30 AM)',
      'T1 Close (4:00 PM)'
  ]

  # Scenario A: The Baseline Fade (Average 8-Quarter Data)
  # Open: +3.86%, Peak: +6.06%, Close: +1.34%
  fade_y = [t0_price, t0_price * 1.0386, t0_price * 1.0606, t0_price * 1.0134]
  plt.plot(x,
           fade_y,
           marker='o',
           label='Baseline Fade (Avg)',
           color='blue',
           linewidth=3)

  # Scenario B: Super-Beat Squeeze (Feb 2024 proxy: +16.4% close)
  # Open: +10%, Peak: +16.4%, Close: +16.4%
  squeeze_y = [t0_price, t0_price * 1.10, t0_price * 1.164, t0_price * 1.164]
  plt.plot(x,
           squeeze_y,
           marker='^',
           label='Super-Beat Squeeze (Tail Risk)',
           color='green',
           linestyle='--',
           linewidth=2)
  plt.fill_between(x,
                   squeeze_y, [y * 0.98 for y in squeeze_y],
                   color='green',
                   alpha=0.1)

  # Scenario C: In-Line Miss (Feb 2025 proxy: -8.47% close)
  # Open: +2.86%, Peak: +3.0%, Close: -8.47%
  plunge_y = [t0_price, t0_price * 1.0286, t0_price * 1.03, t0_price * 0.9153]
  plt.plot(x,
           plunge_y,
           marker='v',
           label='In-Line Miss (Deflationary Plunge)',
           color='red',
           linestyle=':',
           linewidth=2)
  plt.fill_between(x,
                   plunge_y, [y * 1.02 for y in plunge_y],
                   color='red',
                   alpha=0.1)

  plt.title("NVDA Predicted Execution Trajectories (T+1 Models)",
            fontsize=14,
            fontweight='bold')
  ...
  try:
    ...
    if not df_intra.empty:
      ...
      if df_intra.index.tz is None:
        df_intra.index = df_intra.index.tz_localize('UTC').tz_convert(
            'US/Eastern')
      else:
      ...
      t1_open_t = pd.to_datetime('2026-02-26 09:30:00').tz_localize(
          'US/Eastern')
      t1_peak_t = pd.to_datetime('2026-02-26 10:30:00').tz_localize(
          'US/Eastern')
      t1_close_t = pd.to_datetime('2026-02-26 16:00:00').tz_localize(
          'US/Eastern')
      ...
      for t, val in close_series.items():
        ...
        if t <= t1_open_t:
          ...
          frac = (t - t0_t).total_seconds() / (
              t1_open_t - t0_t).total_seconds() if t0_t != t1_open_t else 0
          mx = 0 + frac
        elif t <= t1_peak_t:
          # map t1_open_t -> t1_peak_t to [1, 2]
          frac = (t - t1_open_t).total_seconds() / (
              t1_peak_t -
              t1_open_t).total_seconds() if t1_open_t != t1_peak_t else 0
          mx = 1 + frac
        else:
          # map t1_peak_t -> t1_close_t to [2, 3]
          frac = (t - t1_peak_t).total_seconds() / (
              t1_close_t -
              t1_peak_t).total_seconds() if t1_peak_t != t1_close_t else 0
          mx = 2 + frac
      ...
      if mapped_x and mapped_y:
        plt.plot(mapped_x,
                 ...
                 alpha=0.9,
                 linewidth=3.0,
                 label='Actual 2/26 Intraday Trajectory (5m)')
  except Exception as e:
  ...
  output_path = os.path.join(os.path.dirname(__file__), "plots",
                             "nvda_trajectory_prediction.png")
...
def generate_nvda_fade_plot(nvda_df):
  ...
  if not retrospective.empty:
    ...
    plt.plot(retro_date,
             ...
             alpha=0.8,
             markeredgecolor='none',
             label='2/26 Print (Retrospective)')
...
def generate_intraday_ground_truth_plot():
  try:
    ...
    earnings_time = pd.to_datetime('2026-02-25 16:20:00').tz_localize(
        'US/Eastern')
    plt.axvline(x=earnings_time,
                color='red',
                linestyle='--',
                linewidth=2,
                label='Q4 Earnings Print')

    # Annotate important points
    t0_close_time = pd.to_datetime('2026-02-25 16:00:00').tz_localize(
        'US/Eastern')
    t1_open_time = pd.to_datetime('2026-02-26 09:30:00').tz_localize(
        'US/Eastern')
    t1_close_time = pd.to_datetime('2026-02-26 16:00:00').tz_localize(
        'US/Eastern')
    ...
    plt.title("NVDA Ground Truth Intraday Trajectory (Feb 25 - Feb 26)",
              fontsize=14,
              fontweight='bold')
    ...
    out_path = os.path.join(os.path.dirname(__file__), "plots",
                            "nvda_intraday_ground_truth.png")
    plt.savefig(out_path, dpi=300)
    plt.close()
  except Exception as e:
...
def build_nvda_decision_tree():
  ...
  dot.node('B1',
           ...
           fillcolor='#d4edda',
           fontsize='14',
           fontname='Helvetica-Bold')  # light green
  dot.node('B2',
           'Scenario B:\nSuper-Squeeze (>+10%)',
           style='filled',
           fillcolor='#fff3cd',
           fontsize='14',
           fontname='Helvetica-Bold')  # light gold/yellow
  dot.node('B3',
           'Scenario C:\nFlat / Negative (<+1%)',
           style='filled',
           fillcolor='#f8d7da',
           fontsize='14',
           fontname='Helvetica-Bold')  # light red
  ...
  # Actions B1
  dot.node('C1_A',
           '9:30 AM Open: SELL 5 Shares\n(Capture FOMO / Avoid Crush)',
           shape='ellipse',
           fontsize='14',
           fontname='Helvetica')
  dot.node('C1_B',
           '2:30 PM Fade: BUY ~21 Shares\n(Ride structural T+3 trend)',
           shape='ellipse',
           fontsize='14',
           fontname='Helvetica')
  dot.edge('B1', 'C1_A')
  dot.edge('C1_A',
           'C1_B',
           label='Deploy Combined Cash',
           fontsize='10',
           fontname='Helvetica-Oblique')

  # Actions B2
  dot.node('C2_A',
           '9:30 AM Open: HOLD 5 Shares\n(Do not fight massive momentum)',
           shape='ellipse',
           fontsize='14',
           fontname='Helvetica')
  dot.node('C2_B',
           '2:30 PM Fade: ABORT NVDA BUYS\n(Deploy cash defensively to AMD)',
           shape='ellipse',
           fontsize='14',
           fontname='Helvetica')
  dot.edge('B2', 'C2_A')
  ...
  # Actions B3
  dot.node('C3_A',
           '9:30 AM Open: SELL 5 Shares\n(Liquidate immediately/Stop Loss)',
           shape='ellipse',
           fontsize='14',
           fontname='Helvetica')
  dot.node('C3_B',
           '2:30 PM Fade: ABORT ALL BUYS\n(Hoard cash, wait 3 days)',
           shape='ellipse',
           fontsize='14',
           fontname='Helvetica')
  dot.edge('B3', 'C3_A')
...
def run_full_analysis():
  ...
  if static_content:
    ...
    md_lines.append(
        "# NVDA Earnings Portfolio Playbook (Q4 Fiscal 2026)\n\n*(Error: Static narrative block missing!)*\n\n"
    )
  ...
  if nvda_df is not None:
    ...
    recent.loc[recent['Earnings_Date'] == pd.to_datetime('2026-02-25').date(),
               'Earnings_Date'] = '2026-02-25 (Retrospective)'
    ...
    avg_df = recent[recent['Earnings_Date'] != pd.to_datetime(
        '2026-02-25').date()]
    ...
    avg_iv_df = iv_df[iv_df['Earnings Date'] != pd.to_datetime(
        '2026-02-25').date()]
    md_lines.append(
  ...
  techs = [
      get_technical_indicators(t, os.path.join(MARKET_DATA_DIR, "tickers"))
      for t in tickers
  ]
  ...
  if techs:
    ...
    plot_portfolio_rsi(
        techs,
        os.path.join(os.path.dirname(__file__), "plots", "portfolio_rsi.png"))
    plot_ma200_distance(
        techs,
  ...
  if not pl_df.empty:
    ...
    pl_df_disp['Current Price'] = pl_df_disp['Current Price'].map(
        lambda x: format_num(x, prefix="$"))
    pl_df_disp['Pre-Q4 Basis'] = pl_df_disp['Pre-Q4 Basis'].map(
        lambda x: format_num(x, prefix="$"))
    # Apply formatting for Unrealized_PnL_Pct and Portfolio_Weight_Pct
    if 'Unrealized_PnL_Pct' in pl_df_disp.columns:
      pl_df_disp['Unrealized_PnL_Pct'] = pl_df_disp['Unrealized_PnL_Pct'].map(
          lambda x: format_num(
              x, is_pct=True, is_signed=True, default_nan="NaN"))
    if 'Portfolio_Weight_Pct' in pl_df_disp.columns:
      pl_df_disp['Portfolio_Weight_Pct'] = pl_df_disp[
          'Portfolio_Weight_Pct'].map(
              lambda x: format_num(x, is_pct=True, default_nan="NaN"))
  ...
  if beta_df is not None and not beta_df.empty:
    ...
    for col in beta_df_disp.columns:
      if col != 'Ticker':
        beta_df_disp[col] = beta_df_disp[col].map(
            lambda x: format_num(x, is_pct=True, is_signed=True))
