...
def generate_screening_scatter(df: pd.DataFrame, output_path: str):
  ...
  plt.axvspan(0, q3_eps + 1.5 * iqr_eps, ymin=0.5, ymax=1, alpha=0.08, color='green')

  # Shade the "Value Trap" Quadrant (Positive Discount, Negative EPS Surprise)
  plt.axvspan(q1_eps - 1.5 * iqr_eps, 0, ymin=0.5, ymax=1, alpha=0.08, color='red')
  ...
  for i in range(plot_df.shape[0]):
    plt.text(
      x=plot_df['Last_EPS_Surprise_Pct'].iloc[i] + 0.3,
      y=plot_df['Discount_to_Intrinsic_Value_Pct'].iloc[i] + 0.3,
      s=plot_df['Ticker'].iloc[i],
      fontdict={"color": 'black', "weight": "bold", "size": 8}
    )

  plt.title('Value Screener: Intrinsic Value Discount vs. Last EPS Surprise', fontweight='bold', fontsize=16)
...
def build_decision_tree(df: pd.DataFrame):
  ...
  dot.node('A', f"Intrinsic Value\nScreener Matrix\n({datetime.date.today().strftime('%b %d, %Y')})", shape='box', style='filled', fillcolor='lightblue')

  # Identify extreme groupings if data exists
  deep_value = df[df['Discount_to_Intrinsic_Value_Pct'] > 40]['Ticker'].head(3).tolist()
  fair_value = df[(df['Discount_to_Intrinsic_Value_Pct'] > 0) & (df['Discount_to_Intrinsic_Value_Pct'] <= 40)]['Ticker'].head(3).tolist()
  overvalued = df[df['Discount_to_Intrinsic_Value_Pct'] < -20]['Ticker'].head(3).tolist()
  value_traps = df[(df['Discount_to_Intrinsic_Value_Pct'] > 20) & (df['Last_EPS_Surprise_Pct'] < 0)]['Ticker'].head(3).tolist()

  dot.node('B1', 'Deep Value\n(Discount > 40%)', style='filled', fillcolor='lightgreen')
  dot.node('B2', 'Fair Value\n(Discount 0% - 40%)', style='filled', fillcolor='lightyellow')
  dot.node('B3', 'Overvalued Risk\n(Premium > 20%)', style='filled', fillcolor='lightcoral')
  dot.node('B4', 'Value Traps\n(Discount > 20%, EPS Miss)', style='filled', fillcolor='lightgray')
  ...
  # Dynamic Allocations
  dot.node('C1', f"BUY / CALL LEAPS:\n{', '.join(deep_value) if deep_value else 'No candidates'}", shape='ellipse')
  dot.node('C2', f"HOLD / SELL PUTS:\n{', '.join(fair_value) if fair_value else 'No candidates'}", shape='ellipse')
  dot.node('C3', f"TRIM EXPOSURE:\n{', '.join(overvalued) if overvalued else 'No candidates'}", shape='ellipse')
  dot.node('C4', f"AVOID / SHORT:\n{', '.join(value_traps) if value_traps else 'No candidates'}", shape='ellipse')
...
def generate_report(df: pd.DataFrame):
  ...
  with open(final_report_md, 'w') as f:
    ...
    f.write(f"> *Analyzed **{asset_count} equities** utilizing Benjamin Graham's revised Intrinsic Value formula, substituting contemporary bond yields and trailing EPS. Objective: Identify deep-value dislocations in the market where growth estimates have not kept pace with pricing reality.*\n\n")

    valid_df = df.dropna(subset=['Discount_to_Intrinsic_Value_Pct'])
    if not valid_df.empty:
      top_discount = valid_df.iloc[0]
      f.write(f"- **Highest Margin of Safety:** {top_discount['Ticker']} (Trading at a {top_discount['Discount_to_Intrinsic_Value_Pct']:,.1f}% discount)\n\n")
    ...
    f.write(f"> **Pipeline Diagnostics:** Out of `{asset_count}` tickers analyzed, there are some data gaps that may affect metric coverage:\n")
    f.write(f"> - Missing Intrinsic Value (Often lacking Forward EPS/Growth projection): **{missing_graham}** tickers\n")
    f.write(f"> - Missing EPS Surprise (Missing quarterly expectations): **{missing_eps}** tickers\n")
    f.write(f"> - Missing Current Price data: **{missing_price}** tickers\n\n")

    f.write("## Execution Matrix\n")
    f.write("Based on raw pricing efficiency against terminal growth estimates, the following dynamic decision matrix dictates capital flow.\n\n")
    ...
    f.write("The following scatter plot maps theoretical discount against actual corporate earnings execution. ")
    f.write("Equities in the **upper-right quadrant** represent the holy grail of value investing: severely undervalued companies that are consistently beating consensus earnings.\n\n")
    f.write("![Intrinsic Value Scatter Plot](./plots/intrinsic_value_scatter.png)\n\n")
    f.write("> [!CAUTION]\n")
    f.write("> **The Value Trap:** High-discount equities residing in the *lower-left* quadrant are actively missing earnings, suggesting their 'cheap' valuation is a direct function of collapsing forward guidance rather than market inefficiency.\n\n")

    f.write(f"### 🏆 Top Deep Value Targets\n")
    f.write("*Filtered for positive execution (Surprise > 0) and high margin of safety (Discount > 0).* \n\n")

    # Format dataframe for the markdown table generator
    valid_df = df.dropna(subset=['Discount_to_Intrinsic_Value_Pct', 'Last_EPS_Surprise_Pct'])
    filtered_df = valid_df[(valid_df['Discount_to_Intrinsic_Value_Pct'] > 0) & (valid_df['Last_EPS_Surprise_Pct'] > 0)]
    display_df = filtered_df.head(25).copy()

    if display_df.empty:
      f.write("> *No tickers currently meet the strict Deep Value criteria (Discount > 0 and EPS Surprise > 0).* \n\n")
      display_df = valid_df.head(15).copy() # fallback to just top 15 by discount
...
if __name__ == "__main__":
  ...
  if screener_df.empty:
    logger.error("Loaded dataframe is empty. Ensure `./run_fetch.sh` has pulled data. Exiting.")
    sys.exit(1)
