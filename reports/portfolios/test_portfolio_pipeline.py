...
from reports.portfolios import portfolio_processor, yahoo_portfolio_fetcher
...
class TestPortfolioPipeline(unittest.TestCase):
    ...
    def test_01_fetcher_with_local_json(self, mock_logger):
        ...
        with patch.object(sys, 'argv', ['yahoo_portfolio_fetcher.py', '--local-json', self.mock_json]):
            yahoo_portfolio_fetcher.main()

        # Check files were generated
        active_path = os.path.join(self.test_dir, "tsvs", "example_active_account.tsv")
        inactive_path = os.path.join(self.test_dir, "tsvs", "example_inactive_trust.tsv")
