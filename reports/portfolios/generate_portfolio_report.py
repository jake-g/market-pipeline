...
# Load .env for ACTIVE_TRADING_PORTFOLIOS
...
if env_active:
    ACTIVE_TRADING_PORTFOLIOS = [p.strip() for p in env_active.split(",") if p.strip()]
else:
    logger.warning("ACTIVE_TRADING_PORTFOLIOS not found in .env. Falling back to empty list.")
...
def map_sectors(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['Sector'] = df['Ticker'].map(lambda x: sector_map.get(x, 'Other/Untracked'))
    return df

def create_pie_chart(df: pd.DataFrame, label_col: str, value_col: str, title: str, filename: str, top_n: int = 15):
    ...
    legend_labels = [f'{row[label_col]} ({row[value_col]/plot_df[value_col].sum()*100:.1f}%)' for _, row in plot_df.iterrows()]
    ax.legend(wedges, legend_labels, title=label_col, loc="center left", bbox_to_anchor=(1, 0, 0.5, 1))
...
def generate_report():
    ...
    if not all_tsv_files:
         ...
         all_tsv_files = [
            ...
            if "_combined" not in f and "_meta" not in f and f != "portfolio.json" and "example" not in os.path.basename(f).lower()
         ]
    ...
    if combined_active_rows:
        ...
        active_agg_df['Price'] = active_agg_df.apply(lambda row: round(row['Current_Value'] / row['Quantity'], 2) if row['Quantity'] > 0 else 0, axis=1)
        active_agg_df['Cost_Basis'] = active_agg_df['Current_Value'] - active_agg_df['Unrealized_PnL_Net']
        active_agg_df['Unrealized_PnL_Pct'] = active_agg_df.apply(lambda row: round(row['Unrealized_PnL_Net'] / row['Cost_Basis'] * 100, 2) if row['Cost_Basis'] > 0 else 0, axis=1)
        active_agg_df['Day_Change_Pct'] = active_agg_df.apply(lambda row: round(row['Day_Change_Net'] / (row['Current_Value'] - row['Day_Change_Net']) * 100, 2) if (row['Current_Value'] - row['Day_Change_Net']) > 0 else 0, axis=1)
        active_agg_df = active_agg_df.sort_values(by='Current_Value', ascending=False)
    ...
    if not active_agg_df.empty:
        ...
        report_lines.append(f"- **Active Daily Change:** ${total_active_day:,.2f}\n")

        # Plots
        alloc_plot = create_pie_chart(active_agg_df, 'Ticker', 'Current_Value', "Active Portfolio Holdings", "active_allocation_pie.png")
        sector_agg = active_agg_df.groupby('Sector')['Current_Value'].sum().reset_index()
        sec_plot = create_pie_chart(sector_agg, 'Sector', 'Current_Value', "Active Sector Weighting", "active_sector_pie.png")

        report_lines.append("### Active Visualizations\n")
        if alloc_plot: report_lines.append(f"![Active Holdings]({alloc_plot})")
        if sec_plot: report_lines.append(f"![Active Sectors]({sec_plot})")
        report_lines.append("\n\n### Active Aggregate Holdings\n")

        # Format df slightly for markdown
        display_df = active_agg_df[['Ticker', 'Sector', 'Price', 'Quantity', 'Current_Value', 'Unrealized_PnL_Net', 'Unrealized_PnL_Pct', 'Day_Change_Net', 'Day_Change_Pct']].round(2)
    ...
    if os.path.exists(meta_path):
        ...
        meta_df = meta_df[~meta_df['Portfolio_Name'].str.contains("Example", case=False, na=False)]
    ...
    if not combined_df.empty:
        ...
        all_alloc = create_pie_chart(combined_df, 'Ticker', 'Current_Value', "Total Global Allocation", "all_allocation_pie.png")
        if all_alloc:
            report_lines.append(f"![Total Global Allocation]({all_alloc})\n\n")

        report_lines.append("### All Aggregate Holdings\n")
        display_all = combined_df[['Ticker', 'Sector', 'Price', 'Quantity', 'Current_Value', 'Unrealized_PnL_Net', 'Day_Change_Net']].round(2)
    ...
    logger.info(f"Successfully generated comprehensive Active/Global Markdown report: {report_path}")
